(() => {
  ...
  const translations = {
    ...
    "zh-TW": {
      ...
      "intro-body": '我們深入研究過去的 BTC 重大崩盤事件，發現崩盤前，幣安 BTCUSDT 永續合約的多空比（LSUR）與持倉量（Open Interest）都會出現可偵測的異常訊號。我們已在 2025 Q4 開發了空單過濾器（Short Filter），經歷本次事件後，進一步開發多單過濾器（Long Filter），完成風險防護機制。以下為加入 Filter 的機制介紹。',
      "sf-title": "\u7A7A\u55AE\u904E\u6FFE\u5668",
    ...
    "zh-CN": {
      ...
      "intro-body": '我们深入研究过去的 BTC 重大崩盘事件，发现崩盘前，币安 BTCUSDT 永续合约的多空比（LSUR）与持仓量（Open Interest）都会出现可侦测的异常信号。我们已在 2025 Q4 开发了空单过滤器（Short Filter），经历本次事件后，进一步开发多单过滤器（Long Filter），完成风险防护机制。以下为加入 Filter 的机制介绍。',
      "sf-title": "\u7A7A\u5355\u8FC7\u6EE4\u5668",
